...
import {
  ...
  LogOut, Mic, Sparkles,
  User, Sun, Moon, Plus, History, ChevronDown, MessageCircle
} from 'lucide-react';
...
/* ── Static config ─────────────────────────────────────────────────── */
const MENU_ITEMS = [
  { id:'agriculture', name:'Agriculture', nameHi:'कृषि', icon:Sprout, path:'/agriculture', color:'#15803d', lightBg:'#dcfce7', desc:'Farming & weather' },
...
const DOMAIN_ICONS = { agriculture:'🌾', healthcare:'🏥', education:'📚', schemes:'📋', general:'🤖' };

/* ── DomainSection component ── */
const DomainSection = ({ 
  item, isSidebarOpen, isActive, isExpanded, 
  currentSessionId, onToggle, onNavigate, 
...
const ChatLayout = ({ children, domain = 'general' }) => {
  ...
  const domColor = DOMAIN_COLORS[domain] || DOMAIN_COLORS.general;
  const dtitle = DOMAIN_TITLES[domain] || DOMAIN_TITLES.general;
  ...
  const toggleDropdown = (domainId) => {
    setOpenDropdown(prev => prev === domainId ? null : domainId);
  };
  ...
  const BottomMenu = () => (
    <div style={{ borderTop: '1px solid #f0fdf4', padding: '8px', display: 'flex', flexDirection: 'column', gap: 2 }}>
      ...
      <button onClick={handleLogout} className="cl-btn cl-btn-danger" style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 10, border: 'none', cursor: 'pointer', background: 'transparent', color: '#ef4444', textAlign: 'left' }}>
        <LogOut size={17} />
        {isSidebarOpen && <span style={{ fontFamily: "'Baloo 2',sans-serif", fontWeight: 600, fontSize: 13 }}>Logout · लॉग आउट</span>}
        {!isSidebarOpen && <span className="cl-tip">Logout</span>}
      </button>
    </div>
  );
